...
- [js 基础知识](#js-基础知识)
	- [字符串技巧](#字符串技巧)
		- [时间对比：`时间个位数形式需补 0`](#时间对比时间个位数形式需补-0)
		- [格式化金钱](#格式化金钱)
		- [生成随机 ID](#生成随机-id)
		- [生成随机 HEX 色值](#生成随机-hex-色值)
		- [生成星级评分](#生成星级评分)
		- [操作 URL 查询参数](#操作-url-查询参数)
	- [数值技巧](#数值技巧)
		- [取整：代替正数的 Math.floor()，代替负数的 Math.ceil()](#取整代替正数的-mathfloor代替负数的-mathceil)
		- [补零](#补零)
		- [转数值：只对 null、""、false、数值字符串有效](#转数值只对-nullfalse数值字符串有效)
		- [时间戳](#时间戳)
		- [精确小数](#精确小数)
		- [判断奇偶](#判断奇偶)
		- [精确小数](#精确小数-1)
	...
	- [布尔值运算](#布尔值运算)
		- [短路运算符](#短路运算符)
		- [判断数据类型：undefined、null、string、number、boolean、array、object、symbol、date、regexp、function、asyncfunction、arguments、set、map、weakset、weakmap](#判断数据类型undefinednullstringnumberbooleanarrayobjectsymboldateregexpfunctionasyncfunctionargumentssetmapweaksetweakmap)
		- [是否为空数组](#是否为空数组)
		- [是否为空对象](#是否为空对象)
		- [满足条件时执行](#满足条件时执行)
		- [为非假值时执行](#为非假值时执行)
		- [数组不为空时执行](#数组不为空时执行)
		- [对象不为空时执行](#对象不为空时执行)
		- [函数退出代替条件分支退出](#函数退出代替条件分支退出)
		- [switch/case 使用区间](#switchcase-使用区间)
	- [数组技巧](#数组技巧)
		- [克隆数组](#克隆数组)
		- [合并数组](#合并数组)
		- [去重数组](#去重数组)
		...
		- [截断数组](#截断数组)
		- [交换赋值](#交换赋值)
		- [过滤空值：undefined、null、""、0、false、NaN](#过滤空值undefinednull0falsenan)
		- [异步累计](#异步累计)
		- [数组首部插入成员](#数组首部插入成员)
		- [数组尾部插入成员](#数组尾部插入成员)
		- [统计数组成员个数](#统计数组成员个数)
		...
		- [获取随机数组成员](#获取随机数组成员)
		- [创建指定长度数组](#创建指定长度数组)
		- [创建指定长度且值相等的数组](#创建指定长度且值相等的数组)
		- [reduce 代替 map 和 filter](#reduce-代替-map-和-filter)
	- [对象 object skill](#对象-object-skill)
		- [克隆对象](#克隆对象)
		- [合并对象](#合并对象)
		- [对象字面量：获取环境变量时必用此方法，用它一直爽，一直用它一直爽](#对象字面量获取环境变量时必用此方法用它一直爽一直用它一直爽)
		- [对象变量属性](#对象变量属性)
		- [创建纯空对象](#创建纯空对象)
		- [删除对象无用属性](#删除对象无用属性)
		- [解构对象属性嵌套](#解构对象属性嵌套)
		- [解构对象属性别名](#解构对象属性别名)
		- [解构对象属性默认值](#解构对象属性默认值)
	- [函数 Function Skill](#函数-function-skill)
		- [函数自执行](#函数自执行)
		- [隐式返回值：只能用于单语句返回值箭头函数，如果返回值是对象必须使用()包住](#隐式返回值只能用于单语句返回值箭头函数如果返回值是对象必须使用包住)
		- [一次性函数：适用于运行一些只需执行一次的初始化代码](#一次性函数适用于运行一些只需执行一次的初始化代码)
		- [惰性载入函数：函数内判断分支较多较复杂时可大大节约资源开销](#惰性载入函数函数内判断分支较多较复杂时可大大节约资源开销)
		- [检测非空参数](#检测非空参数)
		- [字符串创建函数](#字符串创建函数)
		- [优雅处理错误信息](#优雅处理错误信息)
		- [优雅处理 Async/Await 参数](#优雅处理-asyncawait-参数)
		- [优雅处理多个函数返回值](#优雅处理多个函数返回值)
	- [DOM Skill](#dom-skill)
		- [显示全部 DOM 边框：调试页面元素边界时使用](#显示全部-dom-边框调试页面元素边界时使用)
		- [自适应页面：页面基于一张设计图但需做多款机型自适应，元素尺寸使用 rem 进行设置](#自适应页面页面基于一张设计图但需做多款机型自适应元素尺寸使用-rem-进行设置)
		- [过滤 XSS](#过滤-xss)
		- [存取 LocalStorage：反序列化取，序列化存](#存取-localstorage反序列化取序列化存)
		- [键盘](#键盘)
	- [<a name="section_es6"></a>](#a-namesection_es6a)
- [ES6 知识](#es6-知识)
	- [<a name="section_ios"></a>](#a-namesection_iosa)
- [IOS 端问题总结](#ios-端问题总结)
	- [ios 下软键盘右上角 `完成` 事件监控：](#ios-下软键盘右上角-完成-事件监控)
		- [input 在 ios 下会被遮住部分](#input-在-ios-下会被遮住部分)
